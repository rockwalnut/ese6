...
{										
    ...
    {
        col <- (i %% (dim(LHD)[2]) + 1);
		randRow <- matrix(0, j_distinct, 2);
		j <- 0;
		ptm <- proc.time();
		phip_try_best <- Inf;

		while(j <= j_distinct)															### randomly picks J distinct element-exchange in column i mod m
		{
			randTemp <- sample(dim(LHD)[1],2);
			temp <- matrix(0,1,2);
			temp[1,] <- c(randTemp[1], randTemp[2]);
	    
			if(checkContain(randRow, temp) == TRUE)
			{
				next;
			}
			else
		...
		{  	
			...
			RAND <- c(randRow[k,1], randRow[k,2], col); 	
			lhd_try <- elementExchenge(LHD, col, randRow[k,1], randRow[k,2]);
			...
			##tol_inn <- 0.001 ;														### modified 1 -> add tol
...
ESEOuter <- function(lhd_ini)															
{
    ...
    while(i < out_iteration)															### outter loop iteration 
    {
		ptm <- proc.time();
        if(flag == 1)
		{	
...
LHDBestESE <- function(run, var)														### write output file
{
	...
	fileName <- paste("c:\\O-ESEdata", substr(date(), 5, 7),substr(date(), 9, 10), ".csv", sep="") 
